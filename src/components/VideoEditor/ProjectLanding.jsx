...
const ProjectLanding = ({
  ...
  const [firestoreReady, setFirestoreReady] = useState(!db); // Skip waiting if no db
  ...
  // Subscribe to data — wait for first Firestore snapshot before rendering projects
  useEffect(() => {
    if (!artistId) return;
  ...
  const projectStats = useMemo(() => {
    ...
    projects.forEach((p) => {
      ...
      const draftCount = [
        ...(createdContent.slideshows || []).filter(
          (s) => !s.isTemplate && nicheIds.has(s.collectionId),
        ),
  ...
  const totalDrafts = useMemo(() => {
    return (
      (createdContent.slideshows || []).filter((s) => !s.isTemplate).length +
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.key === 'Escape') setPreviewingDraft(null);
    };
  ...
  const deleteProjectCore = useCallback(
    async (projectId) => {
      const cols = getUserCollections(artistId);
  ...
  // Batch delete selected projects — deletes from Firestore, updates UI state directly
  const handleBatchDelete = useCallback(async () => {
    ...
    try {
      ...
      log.warn('[BatchDelete] localStorage write skipped (quota):', e.message);
    }
  ...
  const handleQuickSchedule = useCallback(async () => {
    ...
    try {
      await createScheduledPost(db, artistId, {
        ...
        thumbnail:
          previewingDraft.slides?.[0]?.backgroundImage ||
          previewingDraft.slides?.[0]?.imageA?.url ||
          previewingDraft.thumbnail ||
          null,
  ...
  return (
    <div
      className="flex w-full flex-col items-start bg-black px-4 md:px-12 py-8 overflow-y-auto"
      style={{ maxHeight: '100%' }}
    >
      {/* Header */}
      <div className="flex w-full items-start sm:items-center justify-between gap-4 flex-col sm:flex-row">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Button
            ...
            size="medium"
            icon={<FeatherPlus />}
            onClick={() => (onStartWizard ? onStartWizard() : setShowCreateForm(true))}
          >
            New Project
          </Button>
        </div>
      ...
      <div className="grid w-full grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        {!firestoreReady && (
          <div className="col-span-1 sm:col-span-2 flex items-center justify-center py-16">
            <div className="w-6 h-6 border-2 border-zinc-600 border-t-white rounded-full animate-spin" />
          </div>
        )}
        {firestoreReady && projects.length === 0 && !showCreateForm && (
          ...
          <div className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {upcomingPosts.map((post) => {
              ...
              const rawPostUrl =
                ...
                post.thumbnail;
              const firstSlideUrl =
                rawPostUrl && !rawPostUrl.startsWith('blob:') ? rawPostUrl : null;
              return (
                <div
                  key={post.id}
                ...
                >
                  {firstSlideUrl ? (
                    <div
                      className="w-full aspect-[9/16] bg-neutral-100 relative overflow-hidden"
                  ...
                  <div className="flex w-full flex-col gap-1.5 px-3 pb-3">
                    ...
                    {post.editorState?.audio?.name && (
                      <div className="flex items-center gap-1 truncate">
                        <FeatherMusic
                          style={{ width: 11, height: 11, color: '#6366f1', flexShrink: 0 }}
                        />
                        <span className="text-[11px] text-[#6366f1] truncate">
                          {post.editorState.audio.name
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {platforms.map((p) => (
                          <span
                            ...
                            style={{
                              backgroundColor: (PLATFORM_COLORS[p] || '#666') + '22',
            ...
            {[
              ...
              .map((draft) => {
                ...
                const rawUrl = isVideo
                  ...
                  : draft.slides?.[0]?.backgroundImage ||
                    draft.slides?.[0]?.imageA?.url ||
                    draft.slides?.[0]?.url ||
                    draft.thumbnail;
                    ...
                    <div className="flex w-full flex-col gap-0.5 px-3 pb-3">
                      ...
                      {draft.audio?.name && (
                        <div className="flex items-center gap-1 truncate">
                          ...
                          />
                          <span className="text-[11px] text-[#6366f1] truncate">
                            {draft.audio.name.replace(' Audio Extracted', '').replace('.mp3', '')}
                          </span>
                        </div>
                      )}
                    </div>
      ...
      {previewingDraft && (
        ...
        >
          <div
            className="relative flex flex-col rounded-2xl bg-neutral-50 overflow-hidden"
            style={{ maxWidth: '90vw', maxHeight: '85vh', minWidth: 320 }}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-solid border-neutral-200 px-5 py-4">
              <div className="flex flex-col">
                ...
                <span className="text-caption font-caption text-neutral-400">
                  {previewingDraft.slides?.length || 0} slide
                  {(previewingDraft.slides?.length || 0) !== 1 ? 's' : ''}
                </span>
                {previewingDraft.audio?.name && (
                  <div className="flex items-center gap-1 mt-0.5">
                    <FeatherMusic style={{ width: 12, height: 12, color: '#6366f1' }} />
                    <span
                      className="text-[12px] text-[#6366f1] truncate max-w-[200px]"
                      title={previewingDraft.audio.name}
                    >
                      {previewingDraft.audio.name
                        .replace(' Audio Extracted', '')
                        .replace('.mp3', '')}
              ...
              <div className="flex items-center gap-2">
                ...
                {onEditSlideshow && !previewingDraft._isScheduledPost && (
                  <Button
                    ...
                    onClick={() => {
                      setPreviewingDraft(null);
                      onEditSlideshow(previewingDraft);
                    }}
                  >
                    Edit
                  </Button>
            ...
            {/* Slides */}
            <div
              className="flex flex-wrap justify-center gap-3 p-4 overflow-y-auto"
              style={{ maxHeight: 'calc(85vh - 72px)' }}
            >
              {(previewingDraft.slides || []).map((slide, i) => (
                <div
                  key={slide.id || i}
                  className="relative overflow-hidden rounded-lg border border-solid border-neutral-200"
                  style={{
                    width: 180,
                    aspectRatio: '9/16',
      ...
      {showCreateForm && (
        ...
        >
          <div
            className="w-full max-w-md rounded-xl border border-neutral-200 bg-neutral-50 p-6"
            onClick={(e) => e.stopPropagation()}
          >
              ...
              <div className="flex flex-col gap-2">
                ...
                <SubframeCore.DropdownMenu.Root>
                  <SubframeCore.DropdownMenu.Trigger asChild>
                    ...
                    >
                      {newProjectPage
                        ? `@${newProjectPage.handle} · ${newProjectPage.platform}`
                        : 'No page linked'}
                    </Button>
                  </SubframeCore.DropdownMenu.Trigger>
                  <SubframeCore.DropdownMenu.Content
