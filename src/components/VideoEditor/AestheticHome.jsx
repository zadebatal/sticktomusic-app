...
const AestheticHome = ({
  ...
  const [deleteVideoConfirm, setDeleteVideoConfirm] = useState({
    isOpen: false,
    videoId: null,
    videoName: '',
  });
  const [deleteAudioConfirm, setDeleteAudioConfirm] = useState({
    isOpen: false,
    audioId: null,
    audioName: '',
  });
  ...
  const [deleteSlideshowConfirm, setDeleteSlideshowConfirm] = useState({
    isOpen: false,
    slideshowId: null,
    slideshowName: '',
  });
  ...
  return (
    <div style={styles.container}>
      ...
      {isMobile && mobileSidebarOpen && (
        <div style={styles.mobileSidebarOverlay} onClick={() => setMobileSidebarOpen(false)} />
      )}
                  ...
                  <h1 style={styles.categoryTitle}>
                    ...
                    {studioMode && (
                      <span style={styles.modeBadge}>
                        {' '}
                        › {studioMode === 'videos' ? 'Videos' : 'Slideshows'}
                      </span>
                    )}
                  </h1>
              ...
              >
                ...
                <div style={styles.sharedSection}>
                  <div style={styles.sharedLyricBankHeader}>
                    <span style={styles.sharedLabel}>
                      📝 Lyric Bank ({(selectedCategory.lyrics || []).length} songs)
                    </span>
              ...
              <div style={styles.modeView}>
                ...
                >
                  ...
                  <div style={styles.banksColumn}>
                    ...
                    >
                      <div style={styles.bankHeader}>
                        ...
                        <span style={styles.bankName}>
                          Clips ({selectedCategory.videos?.length || 0})
                        </span>
                      </div>
                      <label
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          ...(isMobile
                            ? { padding: '10px 16px', fontSize: '14px', minHeight: '44px' }
                            : {}),
                        }}
                      >
                        <svg
                    ...
                    >
                      <div style={styles.bankHeader}>
                        ...
                        <span style={styles.bankName}>
                          Audio ({selectedCategory.audio?.length || 0})
                        </span>
                      </div>
                      <label
                    ...
                    >
                      <div style={styles.bankHeader}>
                        ...
                        <span style={styles.bankName}>
                          Lyrics ({(selectedCategory.lyrics || []).length})
                        </span>
                      </div>
                      <button
                        style={{
                  ...
                  <div style={styles.actionsColumn}>
                    ...
                    <Button variant="neutral-secondary" className="w-full" onClick={onViewContent}>
                  ...
                  <div style={styles.expandedBank}>
                    ...
                    <div style={styles.assetGrid}>
                      {(selectedCategory.videos || []).length === 0 ? (
                        <label
                          htmlFor="video-bank-input"
                          style={{ ...styles.emptyState, cursor: 'pointer' }}
                        >
                  ...
                  <div style={styles.banksColumn}>
                    ...
                    >
                      <div style={styles.bankHeader}>
                        ...
                        <span style={styles.bankName}>
                          Image A ({(selectedCategory.imagesA || []).length})
                        </span>
                      </div>
                      <label
                    ...
                    >
                      <div style={styles.bankHeader}>
                        ...
                        <span style={styles.bankName}>
                          Image B ({(selectedCategory.imagesB || []).length})
                        </span>
                      </div>
                      <label
                    ...
                    >
                      <div style={styles.bankHeader}>
                        <span style={{ ...styles.bankIcon, color: '#22c55e' }}>🎵</span>
                        <span style={styles.bankName}>
                          Audio ({(selectedCategory.audio || []).length})
                        </span>
                      </div>
                      <label
                        htmlFor="audio-bank-input"
                        ...
                        (selectedCategory.audio || []).map((audio) => (
                          <div key={audio.id} style={styles.audioCard}>
                            ...
                            <div style={styles.audioCardInfo}>
                              ...
                              <div style={styles.audioCardDuration}>
                                {audio.clipDuration
                                  ? `${Math.floor(audio.clipDuration / 60)}:${String(Math.floor(audio.clipDuration % 60)).padStart(2, '0')}`
                                  : '--:--'}
                              </div>
                            </div>
                            <button
                ...
                >
                  ...
                  <div style={styles.categoryCardInfo}>
                    ...
                    <p style={styles.categoryCardMeta}>
                      {category.videos?.length || 0} clips · {category.audio?.length || 0} audio ·{' '}
                      {(category.imagesA?.length || 0) + (category.imagesB?.length || 0)} images
                    </p>
                  </div>
                </button>
              ))}
      ...
      <ConfirmDialog
        ...
        confirmVariant="destructive"
        onConfirm={() => {
          onDeleteSlideshow?.(deleteSlideshowConfirm.slideshowId);
          setDeleteSlideshowConfirm({ isOpen: false, slideshowId: null, slideshowName: '' });
        }}
        onCancel={() =>
          setDeleteSlideshowConfirm({ isOpen: false, slideshowId: null, slideshowName: '' })
        }
    ...
    >
      ...
      <div style={styles.videoNameContainer}>
        {isEditing ? (
          ...
          <span style={styles.videoName} onClick={() => setIsEditing(true)}>
            {video.name || 'Untitled'}
          </span>
        )}
      </div>
...
const AudioItem = ({ audio, onEdit, onDelete, onRename }) => {
  ...
  return (
    <div
      style={{ ...styles.audioItem, ...(audio.isClip ? styles.audioItemClip : {}) }}
      onMouseEnter={() => setShowActions(true)}
      onMouseLeave={() => {
        if (!isEditing) setShowActions(false);
      }}
    >
      ...
      <div style={styles.audioInfo}>
        {isEditing ? (
          ...
          <span style={styles.audioName} onClick={() => setIsEditing(true)}>
            {audio.name}
          </span>
        )}
      ...
      {showActions && !isEditing && (
        <div style={styles.audioActions}>
          <button style={styles.audioActionBtn} onClick={onEdit} title="Edit trim">
            ✂️
          </button>
          <button
          ...
          <button
            style={{ ...styles.audioActionBtn, color: theme.state.error }}
            onClick={onDelete}
            title="Delete"
          >
            ✕
...
const ImageCard = ({ image, onDelete, isMobile = false }) => {
  ...
  return (
    ...
    >
      ...
      {actionsVisible && (
        <button
          style={{ ...styles.imageDeleteBtn, ...(isMobile ? { minWidth: 44, minHeight: 44 } : {}) }}
          onClick={onDelete}
          title="Delete"
        >
          ✕
...
const getStyles = (theme) => ({
  ...
  slideshowLibraryTitle: {
    fontSize: '20px',
    fontWeight: '600',
    color: theme.text.primary,
    margin: 0,
  },
  slideshowLibraryClose: {
    padding: '8px 12px',
    backgroundColor: 'transparent',
    border: `1px solid ${theme.border.default}`,
    borderRadius: '6px',
