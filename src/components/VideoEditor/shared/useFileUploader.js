    ...
    async ({ caption = '', hashtags = '' } = {}) => {
      ...
      const processNext = async () => {
        while (idx < total) {
          ...
          setFiles((prev) =>
            prev.map((f, i) => (i === currentIdx ? { ...f, status: 'uploading' } : f)),
          );
          ...
          try {
            ...
            const { url: cloudUrl } = await uploadFileWithQuota(
              ...
              (pct) => {
                if (!mountedRef.current) return;
                setFiles((prev) =>
                  prev.map((f, i) => (i === currentIdx ? { ...f, progress: Math.round(pct) } : f)),
                );
              },
              {},
              quotaCtx,
            ...
            if (mountedRef.current) {
              setFiles((prev) =>
                prev.map((f, i) =>
                  i === currentIdx ? { ...f, status: 'done', progress: 100 } : f,
                ),
              );
            }
            completed++;
            ...
            if (mountedRef.current) {
              setFiles((prev) =>
                prev.map((f, i) =>
                  i === currentIdx ? { ...f, status: 'error', error: err.message } : f,
                ),
              );
            }
            completed++;
