...
const ContentLibrary = ({
  ...
  draftsTab,
  onDraftsTabChange,
  onRemixDraft, // (draft, remixType) => void — open editor with remix
}) => {
  ...
  // Subscribe to Firestore collections so filter dropdowns stay in sync with adds/deletes.
  // Use the subscription callback data directly (Firestore truth) instead of reading localStorage.
  const [liveCollections, setLiveCollections] = useState(() =>
    artistId ? getCollections(artistId) : [],
  );
  useEffect(() => {
    if (!isDraftsView || !artistId) {
      setLiveCollections(artistId ? getCollections(artistId) : []);
      return;
    }
    if (!db) {
      setLiveCollections(getCollections(artistId));
      return;
    }
    return subscribeToCollections(db, artistId, (cols) => {
      setLiveCollections(cols);
    });
  }, [isDraftsView, db, artistId]);

  // Also re-read when category/collectionFilter change (navigating between views)
  const allCollections = useMemo(() => liveCollections, [liveCollections]);
  const projects = useMemo(
    () => allCollections.filter((c) => c.isProjectRoot === true),
    [allCollections],
  );
  const niches = useMemo(() => {
    if (!artistId || projectFilter === 'all') {
      return allCollections.filter((c) => c.isPipeline === true);
    }
    return allCollections.filter((c) => c.projectId === projectFilter && c.isPipeline === true);
  }, [artistId, allCollections, projectFilter]);

  // Map niche (collection) IDs to their project IDs for filtering
  const nicheProjectMap = useMemo(() => {
    const map = new Map();
    for (const c of allCollections) {
      if (c.isPipeline && c.projectId) map.set(c.id, c.projectId);
    }
    return map;
  }, [allCollections]);

  // Reset filters when project or collection context changes
  // Reset niche filter when project filter changes
  useEffect(() => {
    setNicheFilter('all');
  }, [projectFilter]);
  // Reset filters when collection context changes (navigating away/back)
  useEffect(() => {
    setProjectFilter('all');
    setNicheFilter('all');
    setFilter('all');
  }, [collectionFilter]);
  // If a selected project/niche was deleted, reset to 'all'
  useEffect(() => {
  ...
  useEffect(() => {
    if (nicheFilter !== 'all' && !niches.some((n) => n.id === nicheFilter)) {
      setNicheFilter('all');
    }
  }, [niches, nicheFilter]);
  ...
  React.useEffect(() => {
    ...
    getScheduledPosts(db, artistId)
      .then((posts) => setScheduledPosts(posts.filter((p) => p.status !== 'draft')))
      .catch(() => {});
  }, [isDraftsView, db, artistId]);
  ...
  const items = useMemo(() => {
    const raw = isSlideshow ? category?.slideshows || [] : category?.createdVideos || [];
  ...
  useEffect(() => {
    if (showAudioAssign && db && artistId && audioLibrary.length === 0) {
      getLibraryAsync(db, artistId)
        .then((media) => {
          const audioItems = (media || [])
            .filter(
              (m) =>
                m.type === 'audio' ||
                m.name?.match(/\.(mp3|wav|m4a|aac|ogg)$/i) ||
  ...
  const handleBulkAudioAssign = useCallback(
    async (audioItem) => {
  ...
  // Legacy niches (isPipeline but no projectId) pass through the project filter
  // since we can't determine which project they belong to.
  const applyProjectNicheFilter = useCallback(
    (item) => {
      if (projectFilter !== 'all') {
        const itemProject = item.collectionId ? nicheProjectMap.get(item.collectionId) : null;
        if (itemProject === undefined && item.collectionId) {
          // Niche has no project link — show under all projects rather than hiding
        } else if (itemProject !== projectFilter) {
          return false;
        }
      }
      if (nicheFilter !== 'all') {
  ...
  const filteredScheduledItems = useMemo(
    () => scheduledItems.filter(applyProjectNicheFilter),
    [scheduledItems, applyProjectNicheFilter],
  );
  const filteredPostedItems = useMemo(
    () => postedItems.filter(applyProjectNicheFilter),
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      {/* Header */}
      <div
        className={`flex items-center justify-between px-6 py-5 border-b border-neutral-200 ${isDraftsView && !isMobile ? '!grid !grid-cols-[1fr_auto_1fr] !items-center' : ''} ${isMobile ? '!flex-col !items-stretch !gap-3 !p-4' : ''}`}
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-neutral-100 rounded-lg flex items-center justify-center text-base font-semibold text-neutral-400">
        ...
        <div
          className={`flex items-center gap-2 ${isDraftsView ? 'justify-end' : ''} ${isMobile ? '!flex-col !w-full !gap-2' : ''}`}
        >
          {isDraftsView ? (
      ...
      {isDraftsView && (
        <div
          className={`flex items-center justify-between px-6 py-3 border-b border-neutral-200 ${isMobile ? '!flex-col !items-stretch !gap-2 !px-4' : ''}`}
        >
          {draftTab === 'drafts' ? (
          ...
          <div className="flex items-center gap-4">
              ...
              <label className="flex items-center gap-2 text-neutral-400 text-[13px] cursor-pointer">
                <input
                  ...
                  checked={
                    ...
                    filteredItems.every((v) => selectedVideoIds.has(v.id))
                  }
                  onChange={selectAll}
            ...
            {isDraftsView && projects.length > 0 && (
              <select
                ...
                className="px-3 py-2 bg-black border border-neutral-200 rounded-md text-white text-[13px] cursor-pointer"
            ...
            <div className="px-6 py-4 border-b border-neutral-200 bg-neutral-50">
              <div className="flex items-center justify-between mb-3">
                <h3 className="m-0 text-sm font-semibold text-white">
                  Trash ({trashItems.length} {isSlideshow ? 'slideshow' : 'video'}
                  ...
                  isSlideshow ? (
                    ...
                    >
                      <SlideshowCard
                        ...
                        onExportToDropbox={
                          dropboxConfigured ? () => handleExportToDropbox(item) : null
                        }
                        ...
                        onPost={async () => {
                          if (onViewScheduling && db && artistId) {
                            ...
                            try {
                              ...
                              const banks = item.collectionId
                                ? resolveCollectionBanks(artistId, item.collectionId)
                                : null;
          ...
          {filteredScheduledItems.length === 0 ? (
            ...
            >
              {filteredScheduledItems.map((item) => {
                const postInfo = scheduledPosts.find(
                  (p) =>
                    p.contentId === item.id && (p.status === 'scheduled' || p.status === 'draft'),
                );
                return (
                  <div key={item.id} style={{ position: 'relative' }}>
                    <div
                      className="absolute top-2 right-2 z-10 flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold text-white"
                      style={{ backgroundColor: `${theme.accent.primary}cc` }}
                    >
                      <svg
                        width="10"
                        height="10"
                      ...
                      {postInfo?.scheduledTime
                        ? new Date(postInfo.scheduledTime).toLocaleDateString()
                        : 'Scheduled'}
                    </div>
                    {isSlideshow ? (
                      <SlideshowCard
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        const postToRemove =
                          postInfo || scheduledPosts.find((p) => p.contentId === item.id);
                        if (postToRemove) {
          ...
          {filteredPostedItems.length === 0 ? (
            ...
            >
              {filteredPostedItems.map((item) => {
                const postInfo = scheduledPosts.find(
                  (p) => p.contentId === item.id && p.status === 'posted',
                );
                return (
                  <div key={item.id} style={{ position: 'relative' }}>
                    {isSlideshow ? (
                    ...
                    <div className="absolute top-1.5 left-1.5 bg-green-600/90 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide">
                      Posted
                      {postInfo?.postedAt
                        ? ` ${new Date(postInfo.postedAt).toLocaleDateString()}`
                        : ''}
                    </div>
                  </div>
                );
      ...
      {selectedItems.length > 0 && (
        <div
          className={`flex items-center justify-between px-6 py-4 bg-brand-600 mx-6 mb-4 rounded-xl shadow-[0_4px_20px_rgba(99,102,241,0.5)] ${isMobile ? '!flex-col !gap-3 !mx-3 !mb-3 !px-4 !py-3 !pb-[calc(12px+env(safe-area-inset-bottom,0px))]' : ''}`}
        >
          <div className="flex items-center gap-3">
            <input
              type="checkbox"
              checked={
                filteredItems.length > 0 && filteredItems.every((v) => selectedVideoIds.has(v.id))
              }
              onChange={selectAll}
              className="w-5 h-5 accent-brand-600 cursor-pointer"
            />
          ...
          <div
            className={`flex items-center gap-2 ${isMobile ? '!flex-wrap !justify-center !w-full' : ''}`}
          >
            ...
            {!isSlideshow && selectedItems.length === 1 && (
              <Button
                ...
                onClick={() => setExportingVideo(selectedItems[0])}
              >
                Export
              </Button>
            )}
            {isSlideshow && selectedItems.length >= 2 && onEditMultipleSlideshows && (
              <Button
                ...
                size="small"
                icon={<FeatherEdit2 />}
                onClick={() => onEditMultipleSlideshows(selectedItems)}
              >
              ...
              onClick={async () => {
                if (onViewScheduling && db && artistId) {
                  try {
                    const postsToCreate = selectedItems.map((item) => {
                      ...
                      return {
                        ...
                        thumbnail:
                          item.thumbnail ||
                          (isSlideshow
                            ? item.slides?.[0]?.backgroundImage || item.slides?.[0]?.imageUrl
                            : null) ||
                          null,
                        cloudUrl: item.cloudUrl || null,
                        collectionId: item.collectionId || null,
                    ...
                    const { addManyScheduledPosts } =
                      await import('../../services/scheduledPostsService');
            ...
            >
              Schedule {selectedItems.length} {isSlideshow ? 'Carousel' : 'Post'}
            ...
            >
              <div>
                <div style={{ fontSize: '15px', fontWeight: '600', color: theme.text.primary }}>
                  Assign Audio to {selectedVideoIds.size} Draft
              ...
              <IconButton
                size="small"
                icon={<FeatherX />}
                aria-label="Close audio selection"
                onClick={() => setShowAudioAssign(false)}
              />
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
              {audioLibrary.length === 0 ? (
                ...
                <>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await saveCreatedContentAsync(db, artistId, {
                          videos: [],
                          slideshows: cleared,
                        });
                        if (category?.slideshows) {
                  ...
                  <div
                    style={{ height: '1px', background: theme.border.subtle, margin: '4px 20px' }}
                  />
                  ...
                  {audioLibrary.map((audio) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, overflow: 'hidden' }}>
                        ...
                        {audio.duration && (
                          <div
                            style={{ fontSize: '11px', color: theme.text.muted, marginTop: '1px' }}
                          >
            ...
            >
              <div>
                ...
                <div style={{ color: theme.text.muted, fontSize: 12, marginTop: 2 }}>
                  {previewingSlideshow.slides?.length || 0} slides ·{' '}
                  {previewingSlideshow.status || 'draft'}
                </div>
                {previewingSlideshow.audio?.name && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                    ...
                    <span
                      ...
                      title={previewingSlideshow.audio.name}
                    >
                      {previewingSlideshow.audio.name
                        .replace(' Audio Extracted', '')
                        .replace('.mp3', '')}
              ...
              <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                <Button
                  ...
                  onClick={() => {
                    setPreviewingSlideshow(null);
                    onEditSlideshow?.(previewingSlideshow);
                  }}
                >
                  Edit
                </Button>
              ...
              {(previewingSlideshow.slides || []).map((slide, i) => (
                ...
                >
                  {slide.backgroundImage || slide.thumbnail ? (
                    <img
                      src={slide.backgroundImage || slide.thumbnail}
                      alt={`Slide ${i + 1}`}
                  ...
                  <div
                    style={{
                      ...
                      right: 0,
                      padding: '4px 8px',
                      background: theme.overlay.light,
                      color: theme.text.primary,
                      fontSize: 11,
                      textAlign: 'center',
                    }}
            ...
            {previewingVideo.cloudUrl ? (
              ...
              <>
                <video
                  ...
                  muted={
                    !!(
                      previewingVideo.audio?.url &&
                      !previewingVideo.audio.url.startsWith('blob:') &&
                      !previewingVideo.audio.isSourceAudio
                    )
                  }
                  style={{
                    width: '100%',
              ...
              {previewingVideo.audio?.name && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                  ...
                  <span
                    ...
                    title={previewingVideo.audio.name}
                  >
...
const VideoCard = ({
  ...
  return (
    ...
    >
      <div
        className="absolute top-2 left-2 z-10 min-w-[44px] min-h-[44px] flex items-center justify-center"
        onClick={(e) => e.stopPropagation()}
      >
        <input
          type="checkbox"
      ...
      <div
        className="relative aspect-[9/16] bg-black select-none"
        onClick={() => !isRendering && onPreview?.(video)}
      >
        {video.thumbnail || video.thumbnailUrl ? (
          <ThumbImg
            src={thumbMap?.get(video.id) || video.thumbnail || video.thumbnailUrl}
            alt=""
            className="w-full h-full object-cover"
          />
        ) : video.clips?.[0]?.thumbnail || video.clips?.[0]?.thumbnailUrl ? (
          <ThumbImg
            src={video.clips?.[0]?.thumbnail || video.clips?.[0]?.thumbnailUrl}
            alt=""
            className="w-full h-full object-cover"
          />
        ) : video.clips?.[0]?.url ? (
        ...
        {video.textOverlay && (
          <div className="absolute bottom-[40%] left-1/2 -translate-x-1/2 px-4 py-2 bg-black/30 rounded text-white text-xs font-medium">
            {video.textOverlay}
          </div>
        )}
        ...
        {actionsVisible && !isRendering && (
          <div
            className={`absolute top-2 right-2 flex gap-1.5 ${isMobile ? '!bottom-2 !left-2 !right-2 !top-auto !justify-center !flex-wrap bg-black/30 rounded-md p-1' : ''}`}
          >
            <Button
              variant="neutral-secondary"
              size="small"
            ...
            {needsRendering ? (
              ...
              <>
                ...
                <Button
                  ...
                  size="small"
                  icon={<FeatherSend />}
                  onClick={(e) => handleActionClick(e, onPost)}
                >
                  Post
                </Button>
              </>
      ...
      <div className="px-3 py-2.5 bg-[#171717] flex items-center justify-between gap-2">
        ...
        {video.qcResult && (
          ...
          >
            {video.qcResult.passed ? 'QC ✓' : `QC: ${video.qcResult.issues?.length || 0}`}
          </span>
        )}
      </div>
    </div>
...
// Thumbnail component — downscales to 50% of original resolution, caches data URL
...
const ThumbImg = ({ src, alt = '', className = '' }) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        // 50% of original resolution for sharp card previews
        const scale = 0.5;
  ...
  if (!src) return null;
  return (
    <img src={dataUrl || src} alt={alt} loading="lazy" decoding="async" className={className} />
  );
};
...
const SlideshowCard = ({
  ...
  const getSlideThumb = (slide) => {
    ...
    // Prefer the original image URL for sharp draft previews (only a few slides per card)
    return (
      slide.backgroundImage ||
      slide.imageA?.url ||
      slide.imageA?.localUrl ||
  ...
  return (
    <div
      className={`relative rounded-lg overflow-hidden border border-solid transition-colors ${
        isSelected
          ? 'border-brand-600 shadow-[0_0_0_2px_rgba(99,102,241,0.3)]'
    ...
    >
      ...
      <div
        className="relative bg-[#171717] select-none cursor-pointer"
        onClick={() => onPreview?.()}
      >
        {slideCount > 0 ? (
          <div className="flex w-full gap-px bg-[#171717]" style={{ height: '180px' }}>
        ...
        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            {slideshow.audio?.name && (
              <span
                className="flex items-center gap-1 text-[10px] text-indigo-400 truncate max-w-[120px]"
                title={slideshow.audio.name}
              >
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      if (scheduled > 0) {
        ...
        toastError(
          'No carousels were scheduled. Check that your account has the correct platform IDs configured.',
        );
      }
    } catch (err) {
    ...
    <div style={slideshowPostingStyles.overlay}>
      <div style={slideshowPostingStyles.modal}>
        ...
        <div style={slideshowPostingStyles.preview}>
          <div style={slideshowPostingStyles.previewImages}>
            {slideshows.slice(0, 3).map((slideshow, i) => {
              const previewUrl =
                slideshow.exportedImages?.[0]?.url ||
          ...
          <span style={slideshowPostingStyles.previewText}>
            {slideshows.length} carousel{slideshows.length > 1 ? 's' : ''} • {totalSlides} slide
            {totalSlides !== 1 ? 's' : ''}
        ...
        <div
          ...
          style={{ padding: '16px 20px', borderTop: `1px solid ${theme.border.subtle}` }}
        >
          ...
          >
            {isScheduling
              ? exportProgress || 'Scheduling...'
              : `Schedule ${slideshows.length} Carousel${slideshows.length > 1 ? 's' : ''}`}
          </Button>
        </div>
      </div>
    </div>
