  ...
  return (
    <div style={styles.container}>
      ...
      <main style={styles.main}>
        ...
        {/* Persistent Drafts Bar - always visible in studio */}
        {USE_LIBRARY_SYSTEM && currentArtistId && !showEditor && !showSlideshowEditor && (
          (() => {
            const drafts = libraryCategory || { createdVideos: [], slideshows: [] };
            const draftVideos = (drafts.createdVideos || []).filter(v => v.status === 'draft' || v.status === 'DRAFT');
            const draftSlideshows = (drafts.slideshows || []).filter(s => s.status === 'draft' || s.status === 'DRAFT');
            ...
            return (
              <div style={{
                ...
                left: 0,
                right: 0,
                background: 'linear-gradient(to top, rgba(15,15,25,0.98), rgba(15,15,25,0.92))',
                ...
                <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', whiteSpace: 'nowrap', fontWeight: '600' }}>
                  Drafts ({totalDrafts})
                </span>
                {draftVideos.slice(0, 6).map(v => (
                  <button
                    key={v.id}
                    onClick={() => handleMakeVideo(v)}
                    style={{
                      display: 'flex', alignItems: 'center', gap: '6px',
                      padding: '4px 10px', borderRadius: '6px',
                      background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)',
                      color: '#c4b5fd', fontSize: '11px', cursor: 'pointer', whiteSpace: 'nowrap',
                      flexShrink: 0
                    }}
                  >
                    <span>🎬</span> {v.name || 'Video Draft'}
                  </button>
                ))}
                {draftSlideshows.slice(0, 6).map(s => (
                  <button
                    key={s.id}
                    onClick={() => handleMakeSlideshow(s)}
                    style={{
                      display: 'flex', alignItems: 'center', gap: '6px',
                      padding: '4px 10px', borderRadius: '6px',
                      background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)',
                      color: '#86efac', fontSize: '11px', cursor: 'pointer', whiteSpace: 'nowrap',
                      flexShrink: 0
                    }}
                  >
                    <span>🖼</span> {s.name || 'Slideshow Draft'}
                  </button>
                ))}
                <button
                  onClick={() => {
