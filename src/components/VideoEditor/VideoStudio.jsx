...
const VideoStudio = ({
  ...
  const handleUploadVideos = useCallback(
    async (files) => {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        log(
          `Starting upload ${i + 1}/${files.length}: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`,
        );
      ...
      if (uploadedVideos.length > 0) {
        setCategories((prev) => {
          ...
          log(
            'Categories updated, total videos in category:',
            updated.find((c) => c.id === selectedCategory.id)?.videos.length,
          );
          return updated;
        });
  ...
  const handleUploadAudio = useCallback(
    async (files, trimData = null, customName = null) => {
      if (!selectedCategory) return;
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (trimData) {
            ...
            log(
              `Audio uploaded with trim: ${trimData.startTime.toFixed(1)}s - ${trimData.endTime.toFixed(1)}s`,
            );
          }
      ...
      if (uploadedAudio.length > 0) {
        ...
        setSelectedCategory((prev) =>
          prev ? { ...prev, audio: [...prev.audio, ...uploadedAudio] } : prev,
        );
      }
  ...
  const handleSaveAudioClip = useCallback(
    async (clipData) => {
      if (!selectedCategory) return;
      ...
      try {
        ...
        setCategories((prev) =>
          prev.map((cat) =>
            cat.id === selectedCategory.id ? { ...cat, audio: [...cat.audio, savedClip] } : cat,
          ),
        );

        setSelectedCategory((prev) =>
          prev ? { ...prev, audio: [...prev.audio, savedClip] } : prev,
        );
      } catch (error) {
  ...
  const handleDeleteBankVideo = useCallback(
    async (videoId) => {
      if (!selectedCategory) return;
  ...
  const handleDeleteBankAudio = useCallback(
    async (audioId) => {
      if (!selectedCategory) return;
      ...
      if (audio?.storagePath) {
        ...
        if (audio.size && user?.email)
          decrementStorageUsed(db, user.email, audio.size).catch(() => {});
      }
  ...
  const handleRenameBankVideo = useCallback(
    (videoId, newName) => {
          ...
          cat.id === selectedCategory.id
            ? {
                ...
                videos: cat.videos.map((v) =>
                  v.id === videoId ? { ...v, name: newName.trim() } : v,
                ),
              }
            : cat,
        ),
  ...
  const handleRenameBankAudio = useCallback(
    (audioId, newName) => {
      if (!selectedCategory || !newName.trim()) return;
          ...
          cat.id === selectedCategory.id
            ? {
                ...
                audio: cat.audio.map((a) =>
                  a.id === audioId ? { ...a, name: newName.trim() } : a,
                ),
              }
            : cat,
        ),
  ...
  const handleDeleteBankImage = useCallback(
    async (imageId, bank = 'A') => {
      if (!selectedCategory) return;
      ...
      if (image?.storagePath) {
        ...
        if (image.size && user?.email)
          decrementStorageUsed(db, user.email, image.size).catch(() => {});
      }
